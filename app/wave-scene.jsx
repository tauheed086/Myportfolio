...
export default function WaveScene({ paused, settings }) {
    ...
    useEffect(() => {
        ...
        async function setup() {
            ...
            scene.background = new THREE.Color("#dfe2dd");
            ...
            scene.add(fill);
            const side = window.matchMedia("(max-width: 760px)").matches ? 26 : 40;
            const spacing = 0.81;
          ...
          void main() {
            ...
            const ripples = [{ x: 4, z: 2, age: 0.7, strength: 0.65 }];
            ...
            const onPointer = (event) => {
                ...
                if (distance < 0.13 || performance.now() - lastPointerTime < 35)
                    return;
            ...
            container.addEventListener("pointerdown", onPress, { passive: true });
            ...
            const render = (now) => {
                ...
                lastTime = now;
                if (document.hidden || (pausedRef.current && !dirty && !dirtyRef.current))
                    return;
                dirty = false;
                dirtyRef.current = false;
                const options = settingsRef.current || { speed: 1, amplitude: 1, wireframe: false, color: "#2454ef" };
                material.wireframe = options.wireframe;
                blue.set(options.color);
                if (!pausedRef.current) {
                    idle += delta;
                    autoTimer += delta;
                    ...
                    ripples.forEach(ripple => { ripple.age += delta * options.speed; });
                    while (ripples.length && ripples[0].age > 7)
                        ripples.shift();
                    smoothed.lerp(pointer, 0.04);
                }
                const yaw = smoothed.x * Math.PI * 0.05;
                const scrollProgress = Math.max(-1, Math.min(1, -container.getBoundingClientRect().top / window.innerHeight));
                const pitch = smoothed.y * Math.PI * 0.03 + (pausedRef.current ? 0 : scrollProgress * 0.06);
                ...
                positions.forEach(({ x, z }, i) => {
                    ...
                    displacement = THREE.MathUtils.clamp(displacement / Math.max(1, weightSum) * 0.4 * options.amplitude, -0.8, 0.8);
